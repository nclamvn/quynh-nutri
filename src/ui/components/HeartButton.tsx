...
/** Favorite toggle. Filled rose when on. Stops propagation so it works inside
 *  clickable cards without opening the card. */
export function HeartButton({ dishId, size = 20 }: { dishId: string; size?: number }) {
  ...
  return (
    ...
    >
      <svg width={size} height={size} viewBox="0 0 24 24" fill={on ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.7">
